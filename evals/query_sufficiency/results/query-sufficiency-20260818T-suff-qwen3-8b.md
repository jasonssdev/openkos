...
`qwen3:8b`, 14 documents, 3 run(s), `limit=5`, 120 checks.
...
| --- | ---: | ---: | ---: | ---: | ---: |
| `binary` | 1 of 10 | 0 of 10 | 10 of 10 | **3 of 3** | 1.16 |
| `quote` | 0 of 10 | 0 of 10 | 10 of 10 | **3 of 3** | 1.33 |
...
- **`quote`** — POSITIVE -- zero false refusals across 10 grounded questions, and it catches 3 of 3 that attribution misses, at a median 1.33s added per non-refused query.

## Per-question, the three survivors

`binary`:

    RRR      ¿cuáles son las mejores prácticas de chunking en sistemas RAG?
    RRR      ¿cómo se evalúa la calidad de un sistema de recuperación aumen
    RRR      ¿qué relación hay entre la trazabilidad y la verdad contextual  <-- #753's own question
...
`binary`:
    ...
    .R.      ¿quiénes participaron en las reuniones?  <-- FALSE REFUSAL
