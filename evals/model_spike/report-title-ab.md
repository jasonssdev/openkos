...
# Final state after the change (hand-written, 2026-08-04)

Everything above is the **pre-change baseline**: the D1 anchor A/B that opened issue #377's
investigation, generated by the harness and left untouched as the frozen reference. This section
records where the corpus landed after the four prompt axes and the deterministic twin filter.

Verification run `20260804T191657Z`: `qwen3:8b`, arm `h1`, 18 sources x 3 runs = 54 calls, of which
**52 responded** (2 `OllamaUnavailable` timeouts on a loaded machine, not extraction failures).

| Metric | Baseline (h1, above) | After | Read |
| --- | --- | --- | --- |
| mean objects per run | 1.19 | **1.67** | The 1:1 collapse is gone |
| multi-object rate | 0.09 (8/90) | **0.25 (13/52)** | Nearly triple |
| empty runs | 4.4% (4/90) | 5.8% (3/52) | Slightly up, same order -- #129 is not reopened |
| exact twin alongside genuine objects | routine | **none observed** | What the deterministic filter enforces |

## What changed, per source

Sources that broke out of the cap: `02-how-claude-code-works` (1 -> **5, 5, 5**),
`06-mcp-client` (**5, 5**), `01-what-is-claude-code` (-> 3), `08-the-claude-file` (-> 4),
`call-with-maria` (-> 3: `Person` + `Concept:Apatheia` + `Concept:Dichotomy of Control`).

Twins that dissolved on their own once the rubric asked about candidates instead of the document:
`05-workflow` (H1 "Workflow" -> "Explore, Plan, Code, and Commit Workflow") and
`04-your-first-prompt` (H1 "Your First Prompt" -> "Using Claude Code").

## Twins that remain, and why that is correct

Several single-object sources still carry a title matching their own H1 -- `mcp-launch`
(`Event:MCP Launching`), `06-context-management`, `09-subagents`, `11-hooks`. **These are kept
deliberately.** Each is a genuinely single-subject note whose one subject IS what its title names;
suppressing it would return `[]` for real content and violate the extraction floor. The
unconditional anti-twin rule the proposal originally asked for is unsatisfiable against that floor,
which is why both the spec and the criterion were amended to the conditional form: a twin is
dropped only when a genuine object survives it.

## Residuals (not fixed here)

- **Acronym expansion**: `10-mcp` (H1 "MCP") yields `Concept:Model Context Protocol`. A semantic
  twin that exact-normalized matching deliberately does not catch.
- **Leading article**: `08-the-claude-file` (H1 "The CLAUDE.md File") yielded
  `Concept:CLAUDE.md File` beside three genuine objects -- a near-twin that escapes on the dropped
  "The". Fuzzy matching was rejected as unguessable in code; both classes are documented rather
  than silently approximated.
- **The declared `Decision`** never appears: three targeted wordings over ~28 samples produced
  zero. The model renders the essay-framing choice as `Concept:Dichotomy of Control`. Recorded as
  an 8B-tier limit and tracked separately.
- **Per-run variance is high.** Five real `openkos ingest` runs on the fixture wrote 2, 1, 0, 3 and
  1 objects. Before this change the same fixture produced exactly 1, every time, with zero
  variance -- the cap is genuinely gone, but what replaced it is a distribution, not a guarantee.
