...
_SUBORDINATE_ROLE_MARKERS: Final[dict[str, tuple[str, ...]]] = {
    "part_of": ("part of", "component of", "inside", "within"),
    "member_of": ("member of", "one of", "belongs to"),
    "caused_by": ("caused by", "because of", "result of", "due to", "brought about by"),
    "depends_on": ("depends on", "depend on", "requires", "relies on", "needs"),
    "produced_by": (
        "produced by",
        "authored by",
        "created by",
...
"""Phrases that mark the SUBORDINATE endpoint of each asymmetric type
(issue #807).

All five put the subordinate role on SOURCE -- SOURCE is part_of, member_of,
caused_by, depends_on or produced_by the TARGET -- so a marker whose subject
is the TARGET is the reply arguing for the reverse edge.

ENGLISH ONLY, and that bound is real: the run that opened #807 had a Spanish
rationale, and this table would not have fired on it. It fails closed, so a
non-English rationale is left exactly as it is rather than mis-flagged; the
cost is missed detections, never withdrawn correct answers.

Keyed on EXACTLY `ASYMMETRIC_RELATION_TYPES`, and pinned by a test rather
than restated here: a type added to that vocabulary without a marker entry
would otherwise become silently un-checkable. The lookup below is a
`.get`, so the runtime consequence of that drift is a type this check
declines to examine -- never a `KeyError` in the middle of a run."""


def _normalized(text: str) -> str:
    """Casefolded and accent-stripped, so a title matches its mention in the
    rationale regardless of diacritics."""
    decomposed = unicodedata.normalize("NFKD", text.lower())
    return "".join(c for c in decomposed if not unicodedata.combining(c))


def _title_positions(title: str, text: str) -> list[int]:
    """Offsets in `text` where `title` is identifiable, by whole title first
    and by its distinctive four-plus-character tokens otherwise -- the model
    paraphrases titles rather than quoting them."""
    normalized_text, normalized_title = _normalized(text), _normalized(title)
    # Word-anchored, exactly like the token fallback below (review R3): an
    # unanchored title matches inside a longer word, so `Voice Parts` would
    # be "found" in a rationale that only ever said `Voice Partsong`, and a
    # role marker would then be attributed to a document nobody named.
    whole = [
        m.start()
        for m in re.finditer(rf"\b{re.escape(normalized_title)}\b", normalized_text)
    ]
    if whole:
        return whole
    return [
        m.start()
        for token in re.findall(r"[a-z0-9]{4,}", normalized_title)
        for m in re.finditer(rf"\b{re.escape(token)}\b", normalized_text)
    ]


def _contradicts_direction(
    suggested_type: str | None, rationale: str, source_title: str, target_title: str
) -> bool:
    """Whether `rationale` argues for the REVERSE of the direction
    `suggested_type` asserts (issue #807).

    True only when, inside the SENTENCE carrying a subordinate-role marker
    for the asserted type, the TARGET is named before that marker and the
    SOURCE is not. That is the reply contradicting itself: the type says
    SOURCE holds the subordinate role, the prose says TARGET does.

    The sentence bound is what makes this usable, and it was measured rather
    than assumed. A looser proximity window flagged "The aortic valve is a
    structural component of the human heart, forming part of its anatomy" --
    a CORRECT part_of, caught only because the target is the nearest
    antecedent of a second clause's marker. On a 15-run arm that rule scored
    0.40 precision and would have withdrawn 25 correct suggestions to catch
    17 wrong ones. Bounding the marker to one sentence and requiring the
    target to precede it with no source mention before it scored 1.00
    precision on the arm it was tuned on AND on a held-out arm collected
    afterwards, with 73 correct answers it could have withdrawn and did not.

    Recall is about a fifth of wrong asymmetric answers, which is the honest
    limit of a rule that reads prose without parsing it. It is precision
    this trades for: withdrawing a correct asymmetric edge costs real
    structure, and #613 already established the suggested direction carries
    no evidence, so the graph is not short of caution here."""
    markers = _SUBORDINATE_ROLE_MARKERS.get(suggested_type or "")
    if not markers or not rationale:
        return False
    normalized = _normalized(rationale)
    # A running cursor, not `find` from the start: a rationale that repeats
    # a sentence verbatim would otherwise resolve every later copy back to
    # the FIRST one's offset and analyse that window twice (review R3).
    cursor = 0
    for sentence in re.split(r"(?<=[.!?])\s+", normalized):
        ...
        # Word-anchored, for the reason the title match is (review R3):
        # unanchored, "part of" fires inside "counterpart of" and the check
        # withdraws a correct type on a marker the rationale never used.
        marker_positions = [
            m.start()
            for phrase in markers
            for m in re.finditer(rf"\b{re.escape(phrase)}\b", sentence)
        ]
        ...
        source_at = [
            p - offset
            for p in _title_positions(source_title, rationale)
            if offset <= p < end
        ]
        target_at = [
            p - offset
            for p in _title_positions(target_title, rationale)
            if offset <= p < end
        ]
        for marker in marker_positions:
            target_before = [p for p in target_at if p < marker]
            source_before = [p for p in source_at if p < marker]
            if target_before and not source_before:
                return True
    return False


DIRECTION_WITHDRAWN_NOTE: Final = (
    " [openkos: the specific type was withdrawn -- this rationale argues for "
    "the reverse direction (issue #807)]"
)
"""Appended to a rationale whose type this module withdrew, so the operator
reading the suggestion sees WHY it says `related_to` while the prose argues
a specific structure. Without it the pair reads as the tool contradicting
itself, which is the very complaint #807 is about."""


def _withdraw_contradicted_direction(
    suggested_type: str | None,
    rationale: str,
    src_doc: tuple[str, str],
    tgt_doc: tuple[str, str],
) -> tuple[str | None, str]:
    """Degrade a self-contradicting asymmetric suggestion to `related_to`
    (issue #807), leaving every other suggestion untouched.

    `related_to` rather than `None`: `None` is this module's fail-closed
    degrade for a reply it could not READ, and this reply was read fine --
    it just disagreed with itself about direction. `related_to` is the
    vocabulary's honest abstention, it is symmetric so it asserts no
    direction at all, and the system prompt's own closing guard already
    says an honest `related_to` only declines to say more while a wrong
    `part_of` asserts false structure everything downstream believes."""
    if not _contradicts_direction(suggested_type, rationale, src_doc[0], tgt_doc[0]):
        return suggested_type, rationale
