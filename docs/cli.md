...
#### Extraction: one derived Concept or Entity, or a graceful degrade

Using the model configured in `openkos.yaml`, `ingest` classifies the source's decoded text as one of exactly two types — `Concept` (an idea, topic, theory, term, or framework) or `Entity` (a fallback for a concrete tool, product, or artifact that isn't itself an idea) — or declines to extract anything. `Concept` is always preferred whenever the content fits; `Entity` is used only when it does not.

A successful, validated extraction writes a SECOND document alongside the Source: `bundle/concepts/<slug>.md` for a `Concept`, or `bundle/entities/<slug>.md` for an `Entity`. That document's `provenance` points back at the Source, and its `sensitivity` is inherited verbatim from the Source's own `sensitivity`. Extraction always runs, even under `--auto` or `review: false` — those flags only skip the confirmation PROMPT, never the extraction attempt itself.
...
- the model declines to extract anything, or its reply fails validation (not parseable structured output, a `type` outside `{Concept, Entity}`, or a missing/empty `title`/`description`);
...
A derived document is **create-only**, like the Source: on a re-ingest, if `bundle/concepts/<slug>.md` (or `bundle/entities/<slug>.md`) already exists from a prior successful extraction, it is left completely untouched — no overwrite, no re-extraction — the same way `raw/<name>` is never rewritten. This preserves any hand edits.

Before extraction runs at all, `ingest` checks whether ANY existing `Concept`/`Entity` already cites this source in its `provenance` field. If one does, extraction is skipped entirely for this re-ingest — quietly, with no stderr note, and without even calling the LLM — because a derived object for this source already exists. This check is keyed on **provenance**, not on the derived document's own slug/title, precisely because the LLM's output is not guaranteed deterministic: a re-ingest that produced a slightly different title would otherwise slugify to a different path and pass right by the same-slug guard above, silently creating a second, duplicate derived document (and a duplicate catalog/log entry) for the one source. The same-slug guard remains in place as a secondary safety net for the unrelated case of two different sources colliding on the same slug.

`ingest` computes the raw copy, the Source concept, the staged derived object (if any), and the `index.md`/`log.md` changes in memory first, shows a preview of the proposed changes — listing BOTH the Source and the derived object when one was staged — and only writes after confirmation. When `raw/<name>` already exists, the incoming source's bytes are compared against it before any write: if the bytes are **byte-identical**, `ingest` treats this as an idempotent re-ingest — `raw/<name>` is reused untouched (never re-copied or rewritten) and the Source concept plus `index.md`/`log.md` are regenerated, exiting `0`, regardless of whether the concept already exists. This closes the "forget, then re-ingest" trap: after `openkos forget`, re-ingesting the same source no longer requires deleting `raw/<name>` by hand. Extraction is still attempted on a re-ingest, independently of the Source's own regenerate/fresh status, **as long as no derived object has been produced for this source yet** — it can add a NEW derived object it did not produce the first time (e.g. the LLM was unreachable on the first attempt). If the bytes **differ**, `ingest` still refuses (raw sources are immutable) with a message that distinguishes "differs" from the identical case. A source whose raw copy is absent but whose concept (`bundle/sources/<slug>.md`) already exists is refused as an inconsistent workspace state.
