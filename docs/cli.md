...
**In this version**, Preconditions and Identity run fully; Structure, Metadata, and Contradictions are declared in the stage order but **not yet available in this version** — skipped without prompting and without any model call, and still listed in the end-of-run summary (which always names all five stages with their outcomes) labeled exactly that way.
...
Exit codes: `0` for any completed or declined run — including every declined gate, empty queue, not-yet-available skip, and the Preconditions halt; `1` on a workspace/config failure or a failed mid-walk write; `2` on a usage error; `3` on the post-confirm drift refusal, the one retryable failure (see Conventions).
