...
**Read-only.** Reports what the bundle currently contains, in three sections: **Bundle contents** (source/concept counts from a fresh scan of `bundle/**/*.md`, never from `index.md` alone, so it stays accurate even after an interrupted `ingest`), **Recent activity** (the most recent 5 entries from `log.md`, newest-first), and **Needs attention** (OKF §9 conformance findings — unparseable frontmatter, missing/empty `type` — reused from the same check `ingest`'s generated concepts must pass, plus `lint`'s dangling-reference, dangling-provenance, unextracted-source, below-source-sensitivity, and multi-source-uncovered findings, folded in from the SAME in-memory scan `status` already performs — no extra bundle walk). The section also names a pending exact-title duplicate group count (pointing at `openkos duplicates`), a missing/empty vector index (pointing at `openkos reindex`), and — since #381 — any manifest-gated derived store (`fts.db`, `graph.db`) whose recorded hash no longer matches the bundle, which is the state every curation verb leaves behind and whose only other symptom is a quietly degraded answer. Absence is deliberately not reported as staleness: a freshly `init`ed workspace has no derived store at all, and recommending a refresh of indexes that were never built would be a false alarm. A `failed` extraction is listed here with the same retry command `lint` computes, for example `openkos ingest raw/notes.txt`; a `blocked-by-sensitivity` Source is deliberately never listed (issue #187: not debt, not a retry candidate). It never writes, modifies, or deletes any bundle file.
...
**Read-only.** Answers one narrower question than `status`: not "what is in this bundle", but "which single command should I run next". It ranks five actionable finding kinds by a fixed priority order and prints exactly one runnable command with a one-line reason — or, when none of them fires, one line pointing at `openkos status` for the full report.

Priority order, highest first: (1) missing or empty vector index — `openkos reindex`; (2) derived indexes older than the bundle — `openkos reindex` (#381); (3) an unextracted source (`extraction_status: failed`) — `openkos ingest <resource>`; (4) a descendant below its Source's sensitivity — `openkos backfill-sensitivity`; (5) a pending exact-title duplicate group — `openkos duplicates`. Tiers 1 and 2 recommend the same command and differ only in reason: missing outranks stale, because an absent index blocks retrieval outright while a stale one merely degrades it. Evaluation stops at the first tier with a finding — a lower-ranked tier's finding is never mentioned while a higher one exists — so cost is capped by how far it needs to look: stopping at tier 1 costs nothing beyond the cheapest possible check, and the worst case (reaching tier 5, or finding nothing) still costs less than `status`'s own scan.
