...
Refuses (exit 1) outside an initialized workspace, using the same shared workspace check `ingest`/`status`/`lint` use, before any LLM or index work happens. Retrieval is **hybrid** (hybrid-retrieval-fusion, MVP 2 Slice 3): lexical FTS5 hits and dense `.openkos/vectors.db` hits (via the embedding model configured as `embedding_model`) are combined by reciprocal rank fusion (RRF) into one ranked concept list, which then drives context assembly. `query` is strictly read-only over the vector store — it never creates or writes `.openkos/vectors.db`; run `openkos reindex` first to populate it. A workspace that has never run `reindex` (or whose `vectors.db` is unavailable/corrupt) degrades cleanly to FTS-only retrieval rather than failing.
