...
As shipped, the whole derived layer is five SQLite files under `.openkos/`, all gitignored and none of them a source of truth: **`vectors.db`** (the sqlite-vec embedding store), **`fts.db`** (the FTS5 lexical index), **`graph.db`** (the relation projection), **`findings.db`** (persisted contradiction verdicts, edge suggestions and identity adjudications) and **`insight_questions.db`** (cached source-question embeddings behind `query --save`'s duplicate disclosure). The first three are rebuilt by `openkos reindex`; the findings store is regenerated only by re-running the detectors, because each verdict costs a model call rather than a scan; the question cache refills itself one save at a time. Deleting `vectors.db`, `fts.db` or `graph.db` costs a rebuild, never data. **`findings.db` is the exception**, and it is the reason `openkos purge` prices each store it drops separately: nothing rebuilds a contradiction verdict, an edge suggestion or an identity adjudication, because each one cost the model call that produced it. Losing that store loses judgment history, not an index. The question cache sits between the two — free to lose, refilled one save at a time.
...
- `uv` for dependency and environment management, and `uv_build` as the packaging backend
