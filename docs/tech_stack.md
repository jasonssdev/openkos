...
As shipped, the whole derived layer is four SQLite files under `.openkos/`, all gitignored and none of them a source of truth: **`vectors.db`** (the sqlite-vec embedding store), **`fts.db`** (the FTS5 lexical index), **`graph.db`** (the relation projection) and **`findings.db`** (cached contradiction findings). The first three are rebuilt by `openkos reindex`; the findings cache is regenerated by re-running the detector, because a finding costs a model call rather than a scan. Deleting any of them costs a rebuild, never data.
