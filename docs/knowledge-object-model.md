...
```yaml
id:          # stable identifier, survives file moves
...
Additional fields may be introduced by specialized object types without breaking OKF conformance.
...
sensitivity: private
provenance:
  - raw/pgvector-vs-qdrant.md
  - raw/pgvector-bench-2026-07-14.md
---

# pgvector

pgvector adds vector similarity search to PostgreSQL, using [HNSW](hnsw.md)
indexing. It is compared closely with [Qdrant](qdrant.md).

Latency p95 over 2M vectors: ~40ms under concurrency
(as of 2026-07-14, pgvector-bench). The earlier ~15ms figure was a
single-thread measurement.

## Related
- [Qdrant](qdrant.md) — compared against
- [Vector database selection](vector-database-selection.md) — part of
```

Reading it against the model: the top block is the OKF field set (`type`…`timestamp`) plus the OpenKOS layer (`id`, `status`, `version`, `freshness`, `sensitivity`, `provenance`). The body is human-readable markdown. The `[HNSW](hnsw.md)` and `[Qdrant](qdrant.md)` links are typed relationships that form the graph. The volatile latency figure carries an `as of` stamp (freshness). `provenance` lists the two immutable sources this object was compiled from. Strip the OpenKOS-layer fields and it is still a conformant OKF concept any other tool can read.
...
Objects connect through ordinary markdown links, which form a graph richer than the folder hierarchy. OpenKOS recommends a set of typed relations expressed through link context or frontmatter:
...
These typed relations are what the graph and retrieval layers traverse.
...
**2. Entity resolution and granularity.** Deciding what is *one* object versus many is genuinely hard: is "pgvector" the concept, the tool-entity, and the source page one object or three? When are two mentions the same object? This "boundary problem" is where knowledge graphs get messy. MVP 1 sidesteps it (no formal extraction), but it becomes central in MVP 2. *Stance:* prefer fewer, richer objects over many fragmented ones; make merges reversible; keep entity-resolution decisions reviewable rather than silently automatic.
...
**5. Standard drift.** OKF is minimally opinionated (it requires only `type`); the OpenKOS layer adds more. If our vocabulary diverges too far, interoperability — our core differentiator — erodes, and OKF itself will evolve and need tracking. *Stance:* everything we add lives as ordinary frontmatter and links so a bundle always degrades to conformant OKF; we track the standard rather than fork it, and treat conformance as a tested, ongoing commitment.
