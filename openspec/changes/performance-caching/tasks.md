...
- [ ] 3.25/3.26 (fold-in #4, WAL/busy_timeout + single-commit for `vectors.db`/`vectorstore.py`) — **NOT DONE, DISCREPANCY FLAGGED**: the PR3 delegation message stated "follow-up #4 (WAL/busy_timeout) already landed in PR1 — do NOT redo them." Direct code inspection during this apply session confirmed this is INCORRECT for `vectorstore.py`/`vectors.db` specifically: PR1/PR2 only added WAL/busy_timeout + single-commit to `state/derived.py`'s `open_derived_connection` (used by `fts.db`/`graph.db`), never to `state/vectorstore.py`'s `open_vector_store` (`vectors.db` has NO PRAGMA statements at all, and `reindex()`'s `db.upsert()` calls still commit per-doc, not once per run) — confirmed by grepping the file, zero PRAGMA/WAL hits. Given the coordinator's explicit "do NOT redo" instruction for this batch, this work was NOT performed here; flagged for a follow-up slice/task instead of unilaterally expanding scope against an explicit instruction.
