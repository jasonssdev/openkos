...
- [ ] 2.1 Delete `title = _titleize(src.stem)` at `main.py:1684`.
- [ ] 2.2 Insert derivation immediately after the UTF-8 decode block ends at `main.py:1689` (`raw_content: str | None = src.read_text(encoding="utf-8")` / `except UnicodeDecodeError: raw_content = None`) and strictly before `_build_source_document`'s first call at `main.py:1753`: `derived = None if raw_content is None else source_title.derive_source_title(raw_content); title = derived if derived is not None else _titleize(src.stem)`. **Correction to design.md**: the decode is at `:1689`, not `:1695` — insert after `:1689`, not `:1695`.
- [ ] 2.3 Import `source_title` module at the top of `main.py`; confirm `slug` (computed at `:1646`, consumed by `:1651` and `:1654-1674`) is untouched and still runs before the decode.

## Phase 3: LLM Prompt Consumer Verification

- [ ] 3.1 RED/verify: write or extend an integration test in `tests/unit/cli/test_ingest.py` asserting `_stage_derived_objects(source_title=title)` at `main.py:1764` — and therefore `extraction/concept.py:189`'s `SOURCE TITLE:` prompt line — receives the FINAL derived title (H1/plausible-line value), not the slug, when a candidate is accepted. Use the fake LLM backend already in the test harness and assert on the captured prompt argument, not just the frontmatter.

## Phase 4: Integration Tests — End-to-End Scenarios (`tests/unit/cli/test_ingest.py`)

- [ ] 4.1 RED: `# Introduction to Stoicism` as first non-fenced line -> frontmatter `title`, body `# {title}`, `index.md` bullet, and `log.md` link label all read `Introduction to Stoicism`.
- [ ] 4.2 RED: an H1 inside a fenced code block is ignored; a later real `# Chapter One` outside any fence wins.
- [ ] 4.3 RED: no H1, a title-plausible first line (`Call with Maria Salazar — 2026-07-14` followed by a blank line) becomes the title.
- [ ] 4.4 RED: wrapped prose first line (no blank line after it) is NOT title-plausible; title falls back to `_titleize(src.stem)`.
- [ ] 4.5 RED: a candidate carrying a forbidden character (`[`, `]`, backtick) after normalization falls back to the slug.
- [ ] 4.6 RED: a candidate over 120 chars post-normalization falls back to the slug, with no truncation.
- [ ] 4.7 RED: a well-formed leading `---`...`---` frontmatter block is skipped; its own `title:` key (if present) is not read; a later `# Chapter One` becomes the title.
- [ ] 4.8 RED: an unclosed leading `---` (no later closing `---`) is treated as ordinary content, fails the title-plausible predicate (starts with block syntax), falls back to the slug.
- [ ] 4.9 RED: a binary (`UnicodeDecodeError`) source never calls `derive_source_title` and keeps the slug title.
- [ ] 4.10 RED: an empty or whitespace-only source never calls `derive_source_title` and keeps the slug title.
- [ ] 4.11 RED: `test_reingest_of_identical_bytes_writes_a_byte_identical_source_document` — reuse the `_FixedClock` monkeypatch pattern at `test_ingest.py:2385-2390` (freeze `timestamp`); assert byte-identical Source document across two ingests of the same raw bytes, following the precedent shape of `test_reingest_with_equal_values_writes_byte_identical_output` (`:2401`).
- [ ] 4.12 GREEN: run 4.1-4.11 against the Phase 2 wiring; fix any integration-level defect (the unit suite in Phase 1 should already cover the underlying branch logic).

## Phase 5: Fixture-Churn Verification (own commit, own line-count entry)

- [ ] 5.1 Run the full `tests/unit/cli/test_ingest.py` suite (`uv run pytest tests/unit/cli/test_ingest.py -v`) BEFORE assuming any fixture is safe. Do not rely on the grep finding "no assertions match `Imported [` / `](/sources/`" as a substitute for running the suite — that grep is not proof.
- [ ] 5.2 Identify precisely which of the ~42 content-titled fixtures (34 writing `"content"`, 3 writing `"body"`, plus `"original"`, `"original concept"`, `"malicious"`, 2 `"new content"`) and which of the 46 slug-titled fixtures (`"Some raw notes about self-control."`, `.`-terminated) actually change observed test outcomes, versus which pass unchanged because no assertion reads the derived label.
- [ ] 5.3 Check `tests/unit/vcs/conftest.py:104` (bare-string fixture) against the same predicate; confirm whether it is title-plausible and whether any VCS test asserts on the title it produces.
- [ ] 5.4 IF fixtures require edits to keep assertions correct: make those edits in a SEPARATE commit from the Phase 1/2 implementation commits, and report their line count as its own line in the review forecast — never fold them silently into the implementation diff.
- [ ] 5.5 Add a short code comment at the fixture-churn boundary (e.g. near the first `.`-terminated fixture adjacent to unterminated ones in `test_ingest.py`) explaining why one fixture keeps its slug title and its neighbor does not — the split is a direct consequence of the title-plausible predicate's terminal-punctuation clause, not an inconsistency.

## Phase 6: Final Gate

- [ ] 6.1 `uv run pytest` — full suite green, including `--cov` branch coverage ≥ 90 for `src/openkos/source_title.py`.
- [ ] 6.2 `uv run ruff check .` clean.
- [ ] 6.3 `uv run ruff format --check .` clean.
- [ ] 6.4 `uv run mypy .` clean (strict mode; `derive_source_title` fully typed, no `Any`).
- [ ] 6.5 Confirm no ADR was created (design's ADR gate evaluated both conditions; condition 2 — hard-to-reverse — is not met per the proposal's single-revert rollback plan).
