...
### D4 — `retitle_document(text, *, current_title, new_title) -> str` (revised post-review: surgical patch, not load-and-dump)

Review found the original `load_frontmatter` → mutate dict → `dump_frontmatter` design re-serializes the ENTIRE frontmatter block, verified against the shipped Enchiridion Source: keys re-sorted, `tags` flattened into a block sequence, `timestamp` turned into a `datetime` on reload. **Revised decision: patch the frontmatter surgically, never re-dump it; fail closed on anything the patch cannot handle safely** — the same principle `bundle/index.py:20-31` (D2) already applies to `index.md`. `okf.load_frontmatter` still validates SEMANTICALLY (`metadata["title"] == current_title`); the write is then surgical: `_split_frontmatter_verbatim` (a deliberate separate copy of `index.py`'s split, not a cross-module import) isolates the block byte-for-byte, and `_patch_title_line` rewrites only the top-level `title:` line, raising `ValueError` on a block scalar, an anchor or alias, a duplicated or absent top-level key, a trailing YAML comment, or any value not round-tripping to `current_title` in isolation. Each refusal shape is pinned by its own row in `test_retitle_document_fails_closed_on_unrewritable_title`, asserting that shape's own message so no row can pass because a different guard fired first. The anchor row is load-bearing: its value parses to exactly `current_title`, so the round-trip check passes and only the scalar-shape guard can refuse it — without that guard the anchor definition is deleted while a later alias survives, and the emitted document no longer loads.

A CRLF-framed document is now REFUSED rather than silently normalised. The pre-surgery implementation rewrote it by round-tripping every line ending to LF, which is a whole-file mutation; refusing is the fail-closed reading of "exactly two byte-level edits". Pinned by `test_retitle_document_refuses_a_crlf_framed_document`.

The body's first-line safety assertion is unchanged; the body is now preserved verbatim except that line, so trailing whitespace a re-dump used to drop now survives. `resolve_source_title_backfill` still files every new fail-closed reason under `warned`/`heading-mismatch`, which is no longer an honest label for a frontmatter-shape refusal; a distinct reason token is owed and is tracked as a follow-up, not silently accepted.
