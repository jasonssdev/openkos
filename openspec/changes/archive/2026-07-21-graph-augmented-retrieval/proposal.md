...
### Out of Scope
- Typed/weighted edge PPR (edges unweighted today; uniform PPR; weighting is future).
- Persisted graph store; graph as re-ranker instead of first-class list (keep first-class; seed-exclusion / list-cap is safety valve).
...
- retrieval-fusion: fuse() gains additive optional 3rd typed graph_hits list; equal-weight RRF + rank-only purity preserved; default None = byte-identical current behavior.
...
## Approach
Approach A. Extract pure graph_rank(store, seeds) in new retrieval/graph_retrieve.py owning nx.pagerank(to_digraph(store), personalization=...) on undirected view. Build graph inside answer() (FTS precedent; config-free, rebuilt per run from bundle_dir, no cold-start). Defaults: seeds = top min(limit,5) of initial fuse; alpha=0.85; graph pool = max(limit,10). concept_id identity fully shared — no translation.
...
## Rollback Plan
Revert change branch. fuse() 3rd arg defaults None (byte-identical prior behavior); AnswerResult fields additive with defaults; graph built in-process with no persisted state or migration, so removal leaves FTS + dense exactly as Slice 3 shipped.
...
Artifact store: hybrid. Mirror at openspec/changes/graph-augmented-retrieval/proposal.md. AUTO mode — no interactive question round. Source exploration: #1431.
Session: 000a092f-365f-45a6-9449-341b1fc3dbf3
Project: openkos
Scope: project
Topic: sdd/graph-augmented-retrieval/proposal
Duplicates: 1
Revisions: 1
Created: 2026-07-21 10:58:16
