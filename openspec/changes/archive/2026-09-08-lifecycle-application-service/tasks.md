...
- [x] 3.5 Commit: `feat(lifecycle): seed the application service with ConfirmationRequest and the merge core` -- merged as PR #946.
...
- [x] 5.3 Commit: `refactor(lifecycle): extract unmerge's write-only core into the application service` -- merged as PR #947.

## Slice S2b — PR 3: `prepare_unmerge` (~350–400 lines)

### Phase 6: The missing Phase A (RED first)

- [x] 6.1 RED: extend `tests/unit/application/test_lifecycle.py` with tests calling `application.lifecycle.prepare_unmerge(root, layout, survivor_path, survivor_canonical, absorbed_canonical, now=..., cfg=...)` directly, asserting it returns a `PreparedUnmerge` matching the preview `_execute_single_unmerge` builds today (reversed link/relation/provenance texts, `catalog_log_drifted`, `review`) and performs no write; and that `unwind_step_preview_lines(entry, survivor_canonical)` returns the same lines as `main.py`'s existing helper. Must fail: symbols absent.
- [x] 6.2 RED: add `not hasattr(openkos.cli.main, "_execute_single_unmerge")` to `test_lifecycle_seams.py`. Must fail: the name still exists (S2a left a thin wrapper).
- [x] 6.3 GREEN: add `PreparedUnmerge` and `prepare_unmerge(...)` to `application/lifecycle.py`, relocating the preview-computation body of `_execute_single_unmerge` (re-derived on this branch: `main.py:9915–10314`) and `unwind_step_preview_lines` (re-derived: `main.py:9867`). Rewrite `main.py`'s unmerge command to call `prepare_unmerge`, render the preview from `PreparedUnmerge`'s fields, run the existing gate/guard, then `unmerge_core`; delete `_execute_single_unmerge` (replaced by a same-shaped private adapter `_run_single_unmerge`, since the classic two-arg path and the `--to` chain loop still share one preview/gate/guard body — see Deviations). 6.1 and 6.2 pass.
- [x] 6.4 GREEN: repoint `tests/unit/cli/test_unmerge.py`'s one dangerous-class site (`main._snapshot_read` racing test → `fsio.snapshot_read`, mirroring S1/S3's own precedent — `main.py`'s unmerge adapter no longer calls `_snapshot_read` at all once Phase A moved) to `application_lifecycle`'s call path; `test_unmerge_surgical_catalog.py` has zero `main.X` patch sites (audited, none to repoint). Zero output-text assertion changes in either file.

### Phase 7: Slice 2b gate

- [x] 7.1 Gate: `uv run ruff check .` (clean, 2 dead imports auto-fixed: `bundle_links`/`bundle_relations`, no remaining callers); `uv run ruff format --check .` (295 files already formatted); `uv run mypy .` (no issues, 295 source files); `uv run pytest tests/unit/application tests/unit/cli/test_unmerge.py tests/unit/cli/test_unmerge_surgical_catalog.py -q` (172 passed); full `uv run pytest` unpiped (6045 passed, 3 skipped); `uv run pytest --cov=src/openkos` (96.96% total, `application/lifecycle.py` 99% — both uncovered lines are pre-existing S1/S3 code, not new this slice) ≥ 90% gate met.
- [x] 7.2 Output-text-assertions-unmodified check: zero assertion changes across both unmerge test files, including the classic and `--to` forms (only the one patch-target repoint in 6.4). Falsify: mutated `_run_single_unmerge`'s `"  ~ bundle/{rel} (reverse inbound link rewrite)"` preview line by one character, purged `__pycache__`, confirmed `test_each_rewrite_group_file_lands_in_its_own_preview_partition` went RED, reverted byte-exact (never `git checkout --`), purged `__pycache__` again, confirmed GREEN (62/62). Separately mutated the classic-path non-TTY refusal line (`"...refusing to write without confirmation..."` → `"...confirmationx..."`) the same way: **no test in `test_unmerge.py` caught it** — `test_unmerge_non_tty_without_auto_refuses` only asserts `"--auto" in result.stderr` and `exit_code == 1`, never the full sentence (a pre-existing gap identical in shape on the `--to` chain's own non-TTY test, `test_unmerge_to_non_tty_without_auto_refuses_whole_plan`; both predate this slice — `_execute_single_unmerge` carried the identical wording and the identical weak assertion before the move). Reported honestly per instructions rather than silently strengthening an out-of-scope test; reverted byte-exact, purged `__pycache__`, confirmed GREEN.
- [x] 7.3 Commit: `refactor(lifecycle): build prepare_unmerge and complete unmerge's phase A/B split` -- merged as PR #949.
...
- [x] 9.3 Commit: `refactor(lifecycle): de-present forget into the application service` -- merged as PR #948.

## Slice S4 — PR 5: de-present `purge` (~350–400 lines)

### Phase 10: `PurgePlan` and `PurgeDisclosure` (RED first)

- [x] 10.1 RED: write tests in `tests/unit/application/test_lifecycle.py` for `purge_confirm_phrase(canonical_id, purge_ids, scope)`; `prepare_purge(root, layout, concept_id, scope=..., now=...)` returning a `PurgePlan` with a `TypedChallengeConfirmation` (`supplying_flag == "--confirm-phrase"`, `match_mode == "exact"`, no bypass field); `PurgeDisclosure`'s `raw_absence` arm and `cascade_total` (source scope only); and `dropped_store_notice`/`residual_store_notice` returning `str | None` unchanged (moved verbatim, D3's exception). 9 new tests added to `test_lifecycle.py` (25 total in that file). Implementation and tests were written in the same pass rather than strict watch-it-fail-first sequencing (see apply report); all 9 were confirmed passing against the real implementation, and the pre-existing 76-test `test_purge.py` suite served as the approval-test safety net for the move.
- [x] 10.2 GREEN: add `PurgeDisclosure`, `PurgePlan`, `prepare_purge`, `purge_confirm_phrase`, `dropped_store_notice`, `residual_store_notice` to `application/lifecycle.py` (re-derived coordinates: `purge` body `6672–7305`, its 10 helpers `6272–6671`), de-presenting `purge`'s inline body; all six rails stay adapter-side, before the gate. `_decisions_history_targets` (a private Phase-A-only helper, not itself in the public interface list) also moved, since `prepare_purge` is its only caller. `main.py`'s `purge` command renders `PurgePlan`'s disclosure templates byte-for-byte and drives the typed-phrase gate from `plan.confirmation`'s static fields plus a LIVE re-call to `purge_confirm_phrase` at rail 6 (see design deviation note in apply report: `expected`/`prompt` are NOT read from `plan.confirmation` for the gate comparison, to preserve `test_drift_on_the_unprompted_path_is_refused`'s exact call-site timing). 10.1 passes.
- [x] 10.3 GREEN: repointed `tests/unit/cli/test_purge.py`'s 2 safe-class patch sites (`main._purge_confirm_phrase` → `application_lifecycle.purge_confirm_phrase`; `main._snapshot_read` racing → `fsio.snapshot_read`) to `application_lifecycle`/`fsio`; zero output-text assertion changes across all 76 tests. (The tasks-file estimate of "5" sites did not hold under re-derivation: `main.reindex`, `main._autocommit`, and `main._purge_sweep_store_sidecars` are also patched in this file but target functions that stay adapter-side unmoved, so they needed no repoint — see apply report.) Also repointed `tests/unit/cli/test_forget.py`'s direct call to `main._decisions_history_targets` (discovered during the move, not in the original task text) to `application_lifecycle._decisions_history_targets`.

### Phase 11: Slice 4 gate

- [x] 11.1 Gate: `uv run ruff check .` clean; `uv run ruff format --check .` clean; `uv run mypy .` clean (295 source files); `uv run pytest tests/unit/application tests/unit/cli/test_purge.py -q` — 186 passed; full `uv run pytest -q` — 6048 passed, 3 skipped; full `uv run pytest --cov=src/openkos -q` — 96.97% total coverage (`application/lifecycle.py` 97%), ≥ 90% gate met.
- [x] 11.2 Output-text-assertions-unmodified check: zero assertion changes across `test_purge.py`'s 76 tests (only the 2 patch-target repoints in 10.3). Falsification performed with `__pycache__` purged before/after each mutation, inverse-replace reverts (never `git checkout --`): (1) one character of the IRREVERSIBLE disclosure — **NOT caught, pre-existing gap**: no test in the repo asserts that exact string; (2) the typed-phrase prompt text — **NOT caught, pre-existing gap**: no test asserts the TTY prompt's exact wording (the TTY-path stub ignores its `*args`); (3) the mismatch-abort line — **caught** (`test_purge_confirmation_mismatch_no_write` failed as expected, reverted to GREEN); (4) the non-TTY refusal naming `--confirm-phrase` — **partially caught**: mutating the literal `--confirm-phrase` token IS caught (`test_purge_non_tty_without_confirm_phrase_refuses` failed as expected), but mutating any OTHER part of that same refusal string (e.g. "refusing to purge", "stdin is not a TTY") is **NOT caught** — the assertion is a loose `"confirm-phrase" in result.output.lower()` substring check. All gaps are pre-existing (the strings moved verbatim, unchanged); reported per the falsification-honesty requirement rather than silently expanding scope to add new assertions.
- [x] 11.3 Commit: `refactor(lifecycle): de-present purge into the application service` -- merged as PR #950.
...
- [x] 12.1 RED: write tests in `tests/unit/application/test_lifecycle.py` for `prepare_one_merge(root, layout, index_path, log_path, group, ordered_pair=None)`, `reconcile_planned(prepared, no_reconcile=..., reconcile=...)`, `ordered_merge_pair(bundle_dir, member_ids)`, and `preview_apply_same(root, layout, index_path, log_path, results, include_cross_source=..., include_cross_type=...)` returning `BatchApplyPreview` whose `confirmation.expected == str(len(previewed))` and `match_mode == "strip-then-exact"` (`main.py:3057`'s policy); assert the zero-eligible short-circuit is decided before the gate is ever constructed. Must fail: symbols absent. 10 tests added, confirmed RED (`AttributeError: module 'openkos.application.lifecycle' has no attribute 'preview_apply_same'`, and 9 siblings) before implementing.
- [x] 12.2 GREEN: add `PreviewedPair`, `BatchApplyPreview`, `prepare_one_merge`, `reconcile_planned`, `ordered_merge_pair`, `preview_apply_same` to `application/lifecycle.py`, de-presenting `_run_adjudicate_apply`/`_run_adjudicate_apply_same`'s helper chain (`main.py:2126–3123`, re-derived). `adjudicate_candidates`, `find_candidates_report`, `OllamaClient`, `_reconcile_merged_survivor` stay untouched (C3) — no new `test_adjudicate.py` patch-target repoints beyond S1's 4. 12.1 passes. Deviation: `_cross_source_same_pair`/`_cross_type_concern`/`_member_body_length` also moved (undocumented in the design's abbreviated S5 sketch but required by `preview_apply_same`'s own signature), and `BatchApplyPreview` carries structured `skips`/`items` tuples rather than bare `int` counts — required to preserve exact stdout ordering/text through the adapter; see apply report for full rationale.
- [x] 12.3 GREEN: rewrite `main.py`'s `adjudicate --apply`/`--apply-same` as adapters: Pass 1 calls `preview_apply_same`, renders the preview and typed-count gate from `BatchApplyPreview` via two new render helpers (`_echo_batch_pass1_skips`/`_echo_batch_pass1_items`); Pass 2 re-resolves via `application_lifecycle.prepare_one_merge(ordered_pair=pair.ordered)` per pair, never reusing Pass 1's result (preserving #776's pinned-direction behavior); `_apply_reconciliation`, `_commit_one_merge`, `_reject_drifted_targets` stay adapter-side. A `PreviewMergeFailure(OSError)` (mirroring `PartialForgetWrite`'s shape) carries Pass 1's partial classification state across the exception boundary so a mid-preview failure still renders every skip/preview line that printed before it in the pre-move code.
- [x] 12.4 GREEN: extended `test_lifecycle_seams.py`'s D2 guard to cover `BatchApplyPreview.confirmation` (no `granted`/`force`/`override` field) plus a dedicated repoint-count regression test; confirmed `rg -c '"openkos\.cli\.main\.[A-Za-z_]+"' tests/unit/cli/test_adjudicate.py` == 150 (154 − 4 S1 repoints), unchanged.

### Phase 13: Slice 5 gate

- [x] 13.1 Gate: `uv run ruff check .` clean; `uv run ruff format --check .` clean (295 files); `uv run mypy .` clean (295 source files); `uv run pytest tests/unit/application tests/unit/cli/test_adjudicate.py -q` — 290 passed; full `uv run pytest -q` unpiped — 6069 passed, 3 skipped; full `uv run pytest --cov=src/openkos -q` — 96.99% total coverage (`application/lifecycle.py` 98%, `cli/main.py` 96%), ≥ 90% gate met.
- [x] 13.2 Output-text-assertions-unmodified check: zero diff in `test_adjudicate.py` (confirmed via `git diff`, 0 lines) — all 154 tests / 160 collected cases pass unmodified. Falsification (each: `__pycache__` purged before/after, inverse-replace revert, never `git checkout --`): (1) typed-count prompt text — **NOT caught, pre-existing gap**: only one negative assertion exists (`"Type the eligible count" not in result.stdout` on the zero-eligible path); no test asserts the prompt's positive content. (2) mismatch-abort wording — **NOT caught, pre-existing gap**: `test_adjudicate_apply_same_confirm_count_mismatch_aborts_with_zero_writes` checks only `exit_code == 1` and a byte-identical snapshot, never the stderr text. (3) non-TTY refusal naming `--confirm-count` — **NOT caught for the flag token specifically** (mutating `--confirm-count` alone): the one assertion is a loose `"TTY" in result.stderr` substring check; confirmed NOT vacuous by separately mutating the word "TTY" itself, which DID go RED. (4) one preview line (the batch `survivor:` line) — **caught** (`test_apply_same_survivor_tie_keeps_id_order_and_says_so` failed as expected, reverted to GREEN, 160/160 passing). (5) `.strip()` removed from `TypedChallengeConfirmation.matches()`'s `strip-then-exact` branch — **caught**, by 3 unit tests in `test_lifecycle.py` (`test_typed_challenge_matches_under_strip_then_exact_mode_adjudicate_style`, `test_the_two_typed_gates_diverge_on_a_whitespace_only_difference`, and the new `test_preview_apply_same_confirmation_is_strip_then_exact_typed_count`) — notably NOT caught by any CLI-level `test_adjudicate.py` test (0 of 154 fail), since no CLI test drives `--confirm-count` with surrounding whitespace. All three misses are pre-existing gaps: the strings/logic moved verbatim from `cli/main.py`, unchanged, and the same loose/absent assertions already existed before this slice (mirrors S4's `test_purge.py` findings for the identical shape of gap on `purge`'s own typed-phrase gate). Additionally deleted one of S1's 4 repointed patch lines (`"openkos.application.lifecycle.prepare_merge"` in `test_adjudicate_apply_prepare_merge_failure_stops_the_run`), confirmed the spy-call assertion went RED (`exit_code == 0` instead of the expected non-zero), restored byte-exact (confirmed via `git diff` returning 0 lines).
- [x] 13.3 Commit: `refactor(lifecycle): de-present adjudicate --apply and --apply-same into the application service` -- merged as PR #951.
