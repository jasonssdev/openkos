...
unreachable points to starting the server; a missing model points to
pulling that model tag; an uninitialized workspace points to initializing
it. `doctor` MUST NOT run these commands itself.

#### Scenario: Ollama down shows a start-server remediation

- GIVEN Ollama is not reachable at the configured host
- WHEN `openkos doctor` runs
- THEN the Ollama-reachable check prints `[FAIL]` followed by an indented
  fix line naming the command to start the server
