...
WHEN `answer()` raises an `OllamaError`-family exception (including
`OllamaUnavailable`/`OllamaModelNotFound`) or `FtsUnavailable`, `query` MUST
catch it, print a friendly message to stderr, and exit 1. `query` MUST NOT
let a raw traceback reach the user.

#### Scenario: Ollama backend unreachable

- GIVEN `answer()` raises `OllamaUnavailable`
- WHEN `openkos query "<question>"` is run
