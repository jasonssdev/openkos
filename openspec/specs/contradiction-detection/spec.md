...
`contradicts` relation type (every typed edge is a candidate, except
`derived_from` edges and any pair joined by a resolution edge: `supersedes`,
`reconciled_with` or `revises`).
